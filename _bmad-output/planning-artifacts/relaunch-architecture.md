...
- Trunk routes additionally carry a `troncal` (corridor) grouping absent from the model today — corridor name (e.g. "Calle 26"), zone letter, corridor color, PDF diagram link. **Corrected 2026-08-17 (implementation readiness assessment):** originally marked "deferred, not required for MVP" here — that was wrong. `ux-design-specification.md`'s Color System makes per-corridor trunk-route color a foundational, Phase-1 requirement (Tier 2 of its four-tier color system, used from the very first design decision onward, for exactly the trunk-route subset already in MVP scope per the PRD's own nearby-query scoping). Promoted to a real model below (`Corridor`) rather than left as a prose-only deferral — the data was already confirmed reachable via the live API sample in item 1 above (`"troncal":{"id":10,"nombre":"Calle 26","zona":"K","color":"#D5B079",...}`), so this is a scoping fix, not new research.
