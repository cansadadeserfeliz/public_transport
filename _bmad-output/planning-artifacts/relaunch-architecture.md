...
- **Nearby-query computation (FR1–FR4):** **Server-side**, using PostGIS `ST_DWithin`. **Filtered to `service_tier=troncal` routes and `stop_type=estacion_troncal` stops only** (⚠️ **Made explicit 2026-08-18** — this was previously assumed but never actually stated in this decision; see Data Model Amendment item 2, which already relied on it to justify keeping `service_tier`, and PRD FR1/FR2 as amended 2026-08-18). Matches the PRD's Risk Mitigation Strategy simplification ("index and query only trunk/BRT stops and routes for the nearby view at MVP") — both for the ~1s query-performance target (NFR2) against a much smaller row set (149 stations + trunk routes, vs. 7,618 zonal paraderos + ~500 zonal routes system-wide) and to keep the nearby view aligned with FR9's already-trunk-only live tracking. Zonal expansion is Phase 2, per the PRD's Growth Features. This filter does **not** apply to Route/Stop Browsing (FR5–8) or Sharing (FR17–18), which cover the full catalog regardless of tier. Coordinates are sent from the client to the backend on each request but are never logged or persisted (NFR5) — this is a **policy-level guarantee enforced in code**, not an architectural guarantee by construction (contrast with the client-side alternative that was considered and not chosen). Concretely: the view/serializer handling this query must exclude the raw coordinate values from Django's request logging, and — since Sentry is also in the stack — from Sentry's captured request data (see Infrastructure & Deployment below).
...
- **Last-refreshed disclosure (FR11, added 2026-08-18):** No new field or model needed — `BusStop`/`Route`/`Corridor` already carry `updated_at = models.DateTimeField(auto_now=True)`, and because the refresh strategy above updates matched rows in place (not truncate-and-reinsert), a successful `refresh_gtfs` run bumps `updated_at` on every matched row as a side effect of the upsert. `MAX(Route.objects.aggregate(Max('updated_at')))` (or the equivalent on `BusStop`) therefore gives the last-successful-refresh timestamp for free. Exposed to riders via the Trust Footer (see UX-DR8 / Story 4.1 in `relaunch-epics.md`), alongside the existing data-source attribution line — this is a read at render time, not a stored "last refresh" record, so it's automatically correct with zero extra write path to keep in sync.
...
- **GTFS feed acquisition method (decided 2026-08-18):** `refresh_gtfs` takes a **local file path to a maintainer-downloaded GTFS archive** as its argument; the command and `routes/services/gtfs_refresh.py` never make an outbound HTTP call to fetch it. This resolves a conflict this document previously left unaddressed: earlier language in Integration Points said the feed was "pulled by" the service (implying an automatic fetch), while `relaunch-epics.md` Story 1.3 specified a local-path argument with no internet access — the two contradicted each other. Resolved in favor of the local-path contract: TransMilenio's open data portal (`datosabiertos-transmilenio.hub.arcgis.com`) republishes each GTFS release as a separately-dated document rather than at one fixed URL, so there's no single feed endpoint to hardcode and auto-fetch against without building and maintaining separate "discover the latest release" logic — exactly the kind of extra fragile-external-dependency complexity this project has otherwise deliberately avoided (the same reasoning that already made the refresh trigger manual, not scheduled). The maintainer finds the current dated release on the portal, downloads it, and passes the local path to `refresh_gtfs`; setup docs for this are Story 1.8's responsibility.
- **Caching backend:** Django's local-memory cache (in-process, no separate cache service/container). Chosen for zero added infrastructure at friends-and-family scale; revisit only if the app grows beyond a single worker process. **Size is bounded and does not scale with traffic:** the only thing stored is the live-tracking proxy's short-TTL (~15–30s) entries, keyed **by route, not by user or by request coordinates** — two riders anywhere in the city requesting the same route within the same TTL window share one cache entry. Worst-case size is bounded by the number of distinct trunk routes being actively viewed (a small, fixed ceiling for MVP scope), not by concurrent user count; Django's default `LocMemCache` also self-limits (`max_entries=300`, auto-culls oldest on overflow) as a backstop. The nearby-query (FR1–FR4, keyed by arbitrary lat/lng) is **not** cached at all — it hits the PostGIS spatial index fresh per request — so no cache entry ever scales with rider location.
- **Gunicorn worker count (decided 2026-08-18): exactly 1 worker process.** ⚠️ Made explicit — this was previously an unstated assumption baked silently into the caching decision above ("revisit only if the app grows beyond a single worker process") without ever pinning the actual Gunicorn config that determines it. Django's local-memory cache is per-process; running more than one worker would give each its own independent cache, which doesn't meaningfully worsen memory (the cache is already tiny, see above) but **would** break the transmiapp call-volume-bounding guarantee — N workers means up to N independent cache misses per TTL window instead of one, undermining the PRD's named risk mitigation (avoid drawing attention to the reverse-engineered endpoint) more than it helps request throughput at friends-and-family scale. `Dockerfile`/deployment config must pin `gunicorn --workers 1` explicitly, not leave it at Gunicorn's multi-core-scaling default.
...
- Trunk routes additionally carry a `troncal` (corridor) grouping absent from the model today — corridor name (e.g. "Calle 26"), zone letter, corridor color, PDF diagram link. **Corrected 2026-08-17 (implementation readiness assessment):** originally marked "deferred, not required for MVP" here — that was wrong. `relaunch-ux-design-specification.md`'s Color System makes per-corridor trunk-route color a foundational, Phase-1 requirement (Tier 2 of its four-tier color system, used from the very first design decision onward, for exactly the trunk-route subset already in MVP scope per the PRD's own nearby-query scoping). Promoted to a real model below (`Corridor`) rather than left as a prose-only deferral — the data was already confirmed reachable via the live API sample in item 1 above (`"troncal":{"id":10,"nombre":"Calle 26","zona":"K","color":"#D5B079",...}`), so this is a scoping fix, not new research.
...
Decided 2026-08-17: `BusStop`, `Route`, `RouteStations`, `RouteSchedule`, and `Corridor` all get an explicit `id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)`, replacing Django's default auto-increment integer PK. Raised initially as an enumeration-hardening suggestion and, on its own, not a strong fit for this project — there's no auth and no sensitive data (NFR4), so sequential IDs alone aren't a real exposure. The decision stands on different grounds: URL patterns are already being reworked in this same amendment (`bus_station_detail` → `bus_stop_detail`, `bus-stations/` → `bus-stops/`, per the ripple note below) — this is the point in the project's life where public identifiers can change cheaply, and the point where they get harder to change is once real links have been shared (FR17–18). Choosing opaque, stable-at-creation identifiers now avoids a second URL-scheme break later. Ripple: URL patterns move from `<int:pk>` to `<uuid:pk>` throughout `routes/urls.py`/`routes/api_urls.py`; `gtfs_route_id`/`gtfs_stop_id` (item 1/2 above) remain the *refresh-matching* key, distinct in purpose from the UUID *public-identity* key — a refresh upserts by `gtfs_route_id` and the row keeps its original UUID, so shared links keep resolving across syncs.

**5. Corridor ingestion contract, unmapped-route handling, and color validation (added 2026-08-18).**
Item 2 above left the `Corridor`↔`Route` relationship modeled but not specified as an ingestion contract — a gap, since GTFS has no native corridor concept (`routes.txt` carries no corridor field), yet `gtfs_refresh.py`'s system of record is GTFS, not the live API that originally surfaced the corridor data (per the "Verification still needed" note above, the live API is a cross-check only). Left unresolved, this is exactly the kind of silent gap the ciclovia lifecycle note (item 3) was written to avoid.
- **Mapping source:** `Corridor` rows are **not** derived from either GTFS or a live API call during refresh. Corridors are few (~13) and stable, so they follow the same pattern already decided for the ciclovia rule (item 3): a small, version-controlled fixture maintained in the repo (e.g. `routes/fixtures/corridors.py` or similar), mapping known trunk route codes/`gtfs_route_id`s to a `Corridor` `name`. `gtfs_refresh.py` looks up each `route_mode=transmilenio` route against this maintained mapping to set `Route.corridor`; it never calls the live API and never infers a corridor from GTFS fields that don't carry one.
- **Unmapped trunk routes:** a route with `route_mode=transmilenio` that has no entry in the maintained mapping is **rejected for that refresh run** (not imported with `corridor=null`, and not allowed to silently fall back to a guessed corridor) — consistent with the existing "reject the row, don't guess" precedent already established for unrecognized `day_type` values (item 2). The rejection is logged at `WARNING` (per the existing logging-level convention in Implementation Patterns) and does not fail the rest of the refresh transaction — one route missing a corridor mapping shouldn't block the other ~549 routes from refreshing; it's surfaced for the maintainer to update the fixture and re-run.
- **Corridor color validation:** `Corridor.color` is required and validated as a 6-digit hex string (`^#[0-9A-Fa-f]{6}$`) at the point the fixture is loaded during refresh — enforced by a DB-level `CheckConstraint` (see updated model below) as defense-in-depth, not just application-level validation. A malformed or missing color in the fixture fails that Corridor's load the same way an unmapped route does: rejected and logged, not silently saved with an empty/invalid value, since per-corridor color is a Phase-1 UI requirement (`relaunch-ux-design-specification.md` Color System, Tier 2), not a cosmetic default.
- **Enforcement of trunk-only assignment:** the existing model comment ("null for zonal routes; set for trunk routes only") was prose-only, unlike the equivalent `BusStop.stop_type` rule which has a real `CheckConstraint`. Closed with a matching constraint, added directly to both models below (`corridor_color_valid_hex` on `Corridor.Meta`, `route_corridor_required_for_trunk_only` on `Route.Meta` — see Proposed Data Models).

**Verification still needed before implementation** (not blocking this amendment, but blocking the stories that implement `service_tier`/`schedule`/the natural-key fields): pull an actual GTFS `routes.txt`/`stops.txt`/`calendar.txt`/`calendar_dates.txt` from the `2025-10-28` static feed to confirm `service_tier` sourcing, the `gtfs_route_id`/`gtfs_stop_id` mapping against existing `code`/`cenefa`/`transmilenio_id`, and the full day-type value set plus calendar-exception precedence. The live JSON API sampled above is a useful cross-check but is not itself the system of record — GTFS remains it, per Core Architectural Decisions. The corridor-code-to-name mapping fixture (item 5 above) also needs populating from a real source before `refresh_gtfs` can rely on it — the live API sample gives the shape but not a verified complete list of all ~13 corridors' codes.
...
class Corridor(models.Model):  # NEW — "troncal" grouping, trunk (route_mode=transmilenio) routes only
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    name = models.CharField(max_length=100, unique=True)  # e.g. "Calle 26" — GTFS has no native corridor
    zone_letter = models.CharField(max_length=5)           # concept; corridor count is small (~13) and
    color = models.CharField(max_length=50)                 # stable, so `name` is the refresh natural key,
    pdf_link = models.URLField(default='')                  # not a synthetic gtfs_corridor_id.
    # `name` is matched against a small, version-controlled corridor-mapping
    # fixture during gtfs_refresh.py — never derived from GTFS (no native
    # corridor field) or the live API at refresh time. See Data Model
    # Amendment item 5 (2026-08-18).
...
class Route(models.Model):
    ...
    corridor = models.ForeignKey(       # null for zonal routes; set for trunk routes only
    ...
    class Meta:
        ...
        constraints = [
            # trunk routes must carry a corridor, zonal routes must not —
            # gtfs_refresh.py rejects (not partially loads) any row that
            # violates this; see Data Model Amendment item 5 (2026-08-18)
            models.CheckConstraint(
                check=(
                    models.Q(route_mode=ROUTE_MODE_TRUNK, corridor__isnull=False)
                    | models.Q(route_mode=ROUTE_MODE_ZONAL, corridor__isnull=True)
                ),
                name='route_corridor_required_for_trunk_only',
            ),
        ]
...
**Error Handling:**
- `try/except` boundaries sit at **external-integration points only** — the transmiapp HTTP call, the GTFS archive file read/parse (a local file, not a network fetch — see GTFS feed acquisition method) — not scattered through internal service logic. Internal code trusts its own data once past that boundary (continues the project's existing "trust internal code" philosophy).
...
**Data Boundaries:**
- `routes` app owns `Route`, `RouteSchedule`, `RouteStations`, `Corridor`, `BusStop` (renamed from `BusStation`) models and their PostGIS geometry — the single source of truth for static data.
...
- Official GTFS static feed (ArcGIS Hub / Datos Abiertos Bogotá, `datosabiertos-transmilenio.hub.arcgis.com`) — **manually downloaded by the maintainer**, then read from disk by `routes/services/gtfs_refresh.py`; the service itself makes no outbound HTTP call. ⚠️ **Clarified 2026-08-18** — see the "GTFS feed acquisition method" decision below; this corrects the "pulled by" wording used here previously, which implied an automatic HTTP fetch and conflicted with Story 1.3 in `relaunch-epics.md`.
...
**Data Flow:**
1. Maintainer downloads the current dated GTFS release from the portal, then runs `refresh_gtfs <path-to-archive>` → GTFS feed parsed from the local file → full-replace transaction into `routes` PostGIS tables.
