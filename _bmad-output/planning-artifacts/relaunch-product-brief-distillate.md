...
- **Existing data model** (`routes/models.py`) is a reasonable starting point, not necessarily final: `Route` (name, code e.g. "19-1", route_type enum: transmilenio/alimentador/urbano/complementario/especial, color, schedule as free-text string, map_link, details_link, publication_date/last_update), `BusStation` (name, code, link, address, transmilenio_id unique, cenefa = SITP's official stop code, audio, lat/lon), `RouteStations` join table (route, bus_station, direction 1/2, position = ordered stop sequence per direction). **Superseded 2026-08-17**: this "starting point" was revalidated against current official sources and revised — see `relaunch-architecture.md` § Data Model Amendment (2026-08-17) for the current `BusStop`/`route_mode`+`service_tier`/`RouteSchedule` design. This entry stays as-written since it's a historical record of the pre-rebuild codebase, not a live spec.
