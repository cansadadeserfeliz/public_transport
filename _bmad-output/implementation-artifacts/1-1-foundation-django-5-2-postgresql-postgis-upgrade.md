...
- [x] [Review][Fixed post-hoc, 2026-08-19] No non-root `USER` directive in the Dockerfile — previously deferred, then addressed via a follow-up finding: added `appuser` (UID/GID 1000, matching the host dev user to avoid a docker-compose bind-mount permission mismatch), `chown -R` after `COPY . .`, `USER appuser` before `CMD`. Verified: `whoami`/`id` inside the container confirm non-root; full test suite, `manage.py check`, and `manage.py migrate` all still pass. Caught and fixed a real side effect during validation — stale `htmlcov/`/`.pytest_cache/` directories on the host, left root-owned by earlier pre-fix container runs, blocked the new non-root container from writing to them; cleaned up via a throwaway root container and confirmed clean regeneration with correct host ownership afterward.
